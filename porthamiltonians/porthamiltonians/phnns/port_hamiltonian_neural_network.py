...
class PortHamiltonianNN(DynamicSystemNN):
    """
    Implements a port-Hamiltonian neural network abiding to the port-Hamiltonian formulation
        dx/dt = (S(x) - R(x))*grad[H(x)] + F(x, t)
    where x is the system state, S is the interconection matrix,
    R is the dissipation matrix, H is the Hamiltonian of the system and F is the external ports.
    ...
    ----------
        ...
        external_port_true : The external ports affecting system. Callable taking two tensord as input,
                             x and t, of shape (nsamples, nstates), (nsamples, 1), respectively and returning
                             a tensor of shape (nasamples, nstates). If external_port_true
                             is provided, external_port_est will be ignored.
        ...
        external_port_est  : Estimator for the external ports. Takes a tensor of shape
                             (nsamples, nstates) as input, returning a tensor of shape
                             (nsamples, nstates).
    """
    ...
    def _x_dot(self, x, t, u=None):
        ...
        if (len(S.shape) == 3) or (len(R.shape) == 3):
            dynamics = torch.matmul(S - R, torch.atleast_3d(dH)).reshape(x.shape) + self.external_port(x, t)
        else:
...
def load_phnn_model(modelpath):
    ...
    if external_port_provided:
        ...
        external_port_est = ExternalPortNN(nstates, noutputs, hidden_dim, timedependent,
                                           statedependent, external_port_filter, ttype)
        external_port_est.load_state_dict(metadict['external_port']['state_dict'].copy())
