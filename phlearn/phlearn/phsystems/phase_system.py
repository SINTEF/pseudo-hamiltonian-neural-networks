...
__all__ = ['PhaseEquationSystem', 'init_phase',
           'initial_condition_phase']


class PhaseEquationSystem(ConservativeDissipativeSystem):
    """
    ...
    def __init__(self, x=np.linspace(0,6.-6/100,100), force=None,
        ...
        # check if and where we need the following (to be self.):
        ...
        def dissintegral(u):
            return 1/2*np.sum(((1-np.matmul(Dp,u.T)**2)**2).T, axis=1)
    
        def ham_grad(u):
            return np.zeros_like(u)

        def dissintegral_grad(u):
            return -((2*u @ D1)*((u @ D1)**2-1)) @ D1

        def ham_hessian(u):
            return np.zeros_like(D1)
        
        def dissintegral_hessian(u):
            return -4*np.matmul(D1,
                          np.matmul(D1, np.diag(np.matmul(D1,u)**2-1))
                          + 2*np.matmul(np.diag(np.matmul(D1,u)**2),D1))
...
def init_phase():
    """
    Initialize a standard phase equation system
    ...
    return PhaseEquationSystem(x=x, init_sampler=initial_condition_phase(x))


def initial_condition_phase(x=np.linspace(0,6.-6/100,100)):
    """
